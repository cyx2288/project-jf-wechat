        ...
        MobileArea.prototype = {
            ...
            bindEvent: function () {
                ...
                function popupArea(e) {
                    ...
                    var $inputAdress = $('#credit_input_city');                    //输入匡位置
...
//-----------------------------------------------------------------------------------------------------------------------导入地址选择数据
    selectOptionInput('#credit_city_select select:eq(0)', LAreaData);
...
//------------------------------------------------------------------------------------------------------------------------select选中触发事件
    $('#credit_city_select select').on('mousedown', function (e) {
        if (1 == e.which) {//只允许左击
            showSelect(this);
...
//------------------------------------------------------------------------------------------------------------------------增加后面一项的option
    ...
    function showNextSelect($this) {
        if ($this.index() == 0) {//选择城市
            if ($this.val().indexOf("请选择") < 0) {
                ...
                selectOptionInput('#credit_city_select select:eq(' + thisNum + ')', LAreaData[thisCityNum].child);
            ...
            if ($this.val().indexOf("请选择") >= 0) {
                ...
                if (LAreaData[$this.prev().attr('data-num')].child[thisAreaNum].child) {
                    selectOptionInput('#credit_city_select select:eq(' + thisNum + ')', LAreaData[$this.prev().attr('data-num')].child[thisAreaNum].child);
                    citySelect = 0;
                    //  exchangeButton();
                }
                else {
                    var thisNum = $this.index() + 1;
                    var text_city = $('#credit_city_select select:eq(0)').val() + ',' + $('#credit_city_select select:eq(1)').val();
                    $('#credit_input_city').val(text_city);
                    citySelect = 1;
                    //  exchangeButton();
                    deleteNextSelect('#credit_city_select select:eq(' + thisNum + ')');
                }
            ...
            var text_city = $('#credit_city_select select:eq(0)').val() + ',' + $('#credit_city_select select:eq(1)').val() + ',' + $('#credit_city_select select:eq(2)').val();
            $('#credit_input_city').val(text_city);
            citySelect = 1;
            //  exchangeButton();
        }
    ...
    var area = new LArea();
    area.init({
        'trigger': '#credit_input_city',                                                                                  //触发选择控件的文本框，同时选择完毕后name属性输出到该位置
        'keys': {id: 'id', name: 'name'},                                                                                    //绑定数据源相关字段 id对应valueTo的value属性输出 name对应trigger的value属性输出
        'type': 1,                                                                                                        //数据源类型
        'data': LAreaData                                                                                                 //数据源
    });

    $('#credit_input_city').on('focus', function () {                                                                          //获取焦点后失去焦点，避免误输入
        $(this).blur()
    });
